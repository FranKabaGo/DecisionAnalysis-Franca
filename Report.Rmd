...
#considering income
  ...
  income_timber<-c(rep(0,n_years_first_harvest-1), c(timber_m3*timber_price_n[n_years_first_harvest:n_years])/30)
...
```{r}
plot_cashflow(mcSimulation_object = mc_simulation2, cashflow_var_name = "final_cashflow",  x_axis_name = "Years",
...
```{r}
mcSimulation_cocoa_income <- data.frame(mc_simulation2$x, 
                                 mc_simulation2$y[1])

evpi_cocoatimber <- multi_EVPI(mc = mcSimulation_cocoa_income, 
                   first_out_var = "final_result_cocoa")
plot_evpi(evpi_cocoatimber, decision_vars = "final_result_cocoa")
```

```{r}
options(scipen = 999)

# store Mc simultation outputs
sim1 <- mc_simulation2$y[1]
sim2 <- mc_simulation3$y[1]

sim1_df <- as.data.frame(sim1)
sim2_df <- as.data.frame(sim2)

# Make sure column names match
names(sim2_df) <- names(sim1_df)

# Add simulation label to both
sim1_df$simulation <- "Sim 1"
sim2_df$simulation <- "Sim 2"

# Now bind them
combined <- rbind(sim1_df, sim2_df)
...
library(reshape2)
combined_long <- melt(combined, id.vars = "simulation")

# Plot
ggplot(combined_long, aes(x = value, fill = simulation)) +
  geom_density(alpha = 0.5) +
  facet_wrap(~ variable, scales = "free") +
  labs(title = "Overlay of Distributions", x = "Value", y = "Density")
